package h.com.room_type.controller;
...
import h.com.room_order_list.model.Room_order_listJDBCDAO;
...
//				<input type="file" name="xxx">
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back_end/room_type/update_room_type_input.jsp");
				failureView.forward(req, res);
				return; // 程式中斷
			}
			/*************************** 2.開始修改資料 *****************************************/
...
//		==============給more的按鈕====================
		...
		if("More".equals(req.getParameter("action1"))) {
			Integer room_type_id = Integer.valueOf(req.getParameter("room_type_id1"));
//			System.out.println("有房型編號：" + room_type_id);
			Room_TypeService room_typeSvc = new Room_TypeService();
			Room_TypeVO room_typeVO = room_typeSvc.getOneRoom_Type(room_type_id);
			
			System.out.println("房型名稱: " + room_typeVO.getRoom_type_name());
			req.setAttribute("roomtypeVO", room_typeVO);
			req.setAttribute("room_type_id1", room_type_id);
			if(room_type_id == 1) {
			String url = "/front_end/room/room_type_content1.jsp";
			RequestDispatcher successView = req.getRequestDispatcher(url);// 刪除成功後,轉交回送出刪除的來源網頁
			successView.forward(req, res);
			} 	
		}
...
//			System.out.println("有房型編號：" + room_type_id);
			...
			String url = "/front_end/room/room_type_content2.jsp";
			RequestDispatcher successView = req.getRequestDispatcher(url);// 刪除成功後,轉交回送出刪除的來源網頁
			successView.forward(req, res);
		}
...
//			System.out.println("有房型編號：" + room_type_id);
			...
			String url = "/front_end/room/room_type_content3.jsp";
			RequestDispatcher successView = req.getRequestDispatcher(url);// 刪除成功後,轉交回送出刪除的來源網頁
			successView.forward(req, res);
		}
...
//			System.out.println("有房型編號：" + room_type_id);
			...
			String url = "/front_end/room/room_type_content4.jsp";
			RequestDispatcher successView = req.getRequestDispatcher(url);// 刪除成功後,轉交回送出刪除的來源網頁
			successView.forward(req, res);
		}
		
//		==============給立即訂房的按鈕====================
		if("立即訂房".equals(action)) {
			
			Integer room_type_id = Integer.valueOf(req.getParameter("id"));
			System.out.println("有房型編號：" + room_type_id);
			
			req.setAttribute("room_type_id", room_type_id);
			String url = "/front_end/room_type/room_type_content1.jsp"; //放冠霖要連的
			RequestDispatcher successView = req.getRequestDispatcher(url);// 刪除成功後,轉交回送出刪除的來源網頁
			successView.forward(req, res);
		}
