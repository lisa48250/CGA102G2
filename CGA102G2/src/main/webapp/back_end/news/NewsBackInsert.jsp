...
<!-- 					</ul></li> -->
	...
	<main>
		<div class="right-1">
			<a
				href="${pageContext.request.contextPath}/back_end/news/NewsBackselectAll.jsp"><span>最新消息查詢</span></a>
			<span>/</span> <a
				href="${pageContext.request.contextPath}/back_end/news/NewsBackUpdate.jsp"><span>修改最新消息</span></a>
		</div>
		<div class="font">
			<h1 class="font-1">最新消息新增</h1>
		</div>
	<%-- 	<div class="error">
		...
		<FORM METHOD="post"
			ACTION="${pageContext.request.contextPath}/NewsBack"
			enctype="multipart/form-data">

			<div class="title">
				<label for="title">標題:</label> <input id="title" type="text"
					name="title" value="${news_listVO.news_id_title}">	 	
			</div>
			...
			<div class="content">
				...
				<textarea name="editor1" id="editor1" cols="50" rows="10">${news_listVO.news_id_description}</textarea>
				<%-- 	<p>${param.editor1}</p> --%>
				<script>
			...
			<div class="date">
				<label for="date">日期:</label> <input id="date" type="date"
					name="date" value="${news_listVO.news_id_date}">
				<%-- <p>${param.date}</p> --%>
			</div>
			...
			<div class="pic">
				<label>圖片:</label> <img class="newimg"
					src="${pageContext.request.contextPath}/NewsReader?news_id=${list.news_id}">
				<input type="file" value="選擇檔案" name="pic" value="${news_listVO.news_id_photo}">
				<%-- 	<p>${param.pic}</p> --%>
			</div>
	...
	<script>
	const { body } = document;
	const photo = document.getElementsByClassName("pic")[0];
