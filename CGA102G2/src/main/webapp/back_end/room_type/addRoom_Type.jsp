...
%>
<%-- <%= room_typeVO==null %>--${room_typeVO.getDeptno()}--${room_typeVO.deptno}--<!-- 與100行有關 --> --%>
...
<div id = "tableall">
	...
	<FORM METHOD="post" ACTION="${pageContext.request.contextPath}/room_type.do" id="form1"
		enctype="multipart/form-data">
		<table id="table-2">
			<tr>
				<td>➕ 房型名稱:<font color=red><b>*</b></font></td>
				<td><input type="TEXT" name="room_type_name" size="45"
					value="<%=(room_typeVO == null) ? "日式四人房" : room_typeVO.getRoom_type_name()%>" /></td>
			</tr>
			<tr>
                <td>➕ 房型數量:<font color=red><b></b></font></td>
                <td><input type="TEXT" name="room_type_amount" size="45"
                    value="<%=(room_typeVO == null) ? "1" : room_typeVO.getRoom_type_amount()%>" /></td>
            </tr>
            <tr>
                <td>➕ 房型說明:<font color=red><b></b></font></td>
                <td><input type="TEXT" name="room_type_content" size="1000"
                    value="<%=(room_typeVO == null) ? "<h4>30坪 兩大床（Queen size）</h4> 設施 :<br>吹風機、戶外半露天池、乾濕分離浴室、TV、WIFI、熱水壺、冷暖氣機、礦泉水、泡麵、飲料、<br>吊扇、咖啡包、茶包、棉棒、牙籤、浴帽、大小毛巾、牙刷、面膜、電冰箱" : room_typeVO.getRoom_type_content()%>" /></td>
            </tr>

			<%-- <tr>
		<td>上下架狀態:(輸入0上架或1下架)</td>
		<td><input type="TEXT" name="room_sale_status" size="10"
			 value="<%= (room_typeVO==null)? "1" : room_typeVO.getRoom_sale_status()%>" /></td>
	</tr> --%>
			...
			<tr>
				<td>➕ 評論總覽:</td>
				<td><input type="TEXT" name="room_total_review" size="1000"
					value="<%=(room_typeVO == null) ? "請輸入評論" : room_typeVO.getRoom_total_review()%>" /></td>
			</tr>
			<tr>
				<td>➕ 房型照片:<font color=red><b>*</b></font></td>
				<!-- <td><input type="file" name="room_type_pic" accept="image/*"></td> -->
			    <td><input class="up" type="file" name="room_type_pic" size="45" accept="image/*" />
		        <img class="room_img" name="room_type_pic" src="${pageContext.request.contextPath}/Room_Type_Reader?id=${room_typeVO.room_type_id}" style="height:80px; width:160px; left: 410px; bottom: 35px;  position: absolute; "></td>
	 		</tr>
			<tr>
				<td>➕房型價格:<font color=red><b>*</b></font></td>
				<td><input type="TEXT" name="room_type_price" size="45"
					value="<%=(room_typeVO == null) ? "9800" : room_typeVO.getRoom_type_price()%>" /></td>
			</tr>
