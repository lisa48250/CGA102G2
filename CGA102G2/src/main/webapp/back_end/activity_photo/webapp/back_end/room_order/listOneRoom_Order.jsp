...
<!-- 					</ul></li> -->
	...
	<!-- ==================* 主畫面區 *================== -->
	<main>
		<div id="tableall" style="height: 480px">
			...
			<table id="table-2" style="margin-left: 156px;">
				<tr>
					...
					<td class="value">${room_orderVO.room_order_id}</td>
				</tr>
				<tr>
					<td class="name">➕ 會員編號</td>
					<td class="value">${room_orderVO.member_id}</td>
				<tr>
					<td class="name">➕ 訂單日期</td>
					<td class="value"><fmt:formatDate value="${room_orderVO.order_date}"
							pattern="yyyy-MM-dd HH:mm:ss" /></td>
				</tr>
				<tr><td class="name">➕ 訂單狀態</td>
                    <%-- <td class="value">${room_orderVO.room_order_status}</td> --%>
                    <c:if test="${room_orderVO.room_order_status == 0}"><td class="value">已結單</td></c:if>
                    <c:if test="${room_orderVO.room_order_status == 1}"><td class="value">已入住</td> </c:if>
                    <c:if test="${room_orderVO.room_order_status == 2}"><td class="value">未入住</td>  </c:if>
                </tr>
				<tr>
					<td class="name">➕ 總金額</td>
					<td class="value">${room_orderVO.room_charge}</td>
				</tr>
				<tr>
					<td class="name">➕ 入住日期</td>
					<td class="value"><fmt:formatDate value="${room_orderVO.check_in_date}"
							pattern="yyyy-MM-dd" /></td>
				</tr>
				<tr>
					<td class="name">➕ 退房日期</td>
					<td class="value"><fmt:formatDate value="${room_orderVO.check_out_date}"
							pattern="yyyy-MM-dd" /></td>
				</tr>

				<%-- <tr>
			<td>${room_orderVO.room_order_id}</td>
			<td>${room_orderVO.member_id}</td>
			<td><fmt:formatDate value="${room_orderVO.order_date}"
					pattern="yyyy-MM-dd HH:mm:ss" /></td>
			<td>${room_orderVO.room_order_status}</td>
			<td>${room_orderVO.room_charge}</td>
			<td><fmt:formatDate value="${room_orderVO.check_in_date}"
					pattern="yyyy-MM-dd" /></td>
			<td><fmt:formatDate value="${room_orderVO.check_out_date}"
					pattern="yyyy-MM-dd" /></td>
		</tr> --%>
			</table>
		</div>
