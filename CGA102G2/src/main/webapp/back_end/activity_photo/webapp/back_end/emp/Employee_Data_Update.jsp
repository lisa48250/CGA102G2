...
<%
  EmployeeVO employeeVO_edit = (EmployeeVO) request.getAttribute("employeeVO_edit"); //EmpServlet.java (Concroller) 存入req的empVO物件 (包括幫忙取出的empVO, 也包括輸入資料錯誤時的empVO物件)
%>
...
<body>
    ...
    <aside class="aside">
        <nav class="nav">
            ...
            <ul class="nav_list">
                ...
                <c:choose>
                <c:when test="${employeeVO.job_id == 1001}">
 ...
 <FORM METHOD="post" ACTION="${pageContext.request.contextPath}/Employee_Servlet" enctype="multipart">       
    ...
    <div class="employee_no">員工編號:<%=employeeVO_edit.getEmp_no()%></div>
...
<!--         <b class="p5"></b> -->
      ...
      <div class="input2">
        <input type="text" name="emp_name" placeholder="更改員工姓名" value="${employeeVO_edit.emp_name}">
    </div>
    ...
    <div class="input4">
        <input type="text" name="emp_account" placeholder="更改帳號" value="${employeeVO_edit.emp_account}">
    </div>
    <div class="input5">
        <input type="text" name="emp_password" placeholder="更改密碼" value="${employeeVO_edit.emp_password}">
    </div>
	
   <input type="hidden" name="emp_no" value="${employeeVO_edit.emp_no}">
   <input type="hidden" name="action" value="update">
   <input style="cursor:pointer;" class="change"  type="submit" value="確認修改">
