...
NewsPostVO newspostVO = (NewsPostVO) request.getAttribute("newspostVO"); //EmpServlet.java(Concroller), 存入req的empVO物件
%>
...
<table class="newspostbody">
	<tr>
		...
		<tr>
			<td ><%=newspostVO.getNewsPostId()%></td>
		<td><img class="news_img" name="newsPostId" src="${pageContext.request.contextPath}/NewsPost_Reader?newsPostId=${newspostVO.newsPostId}"></td>
			<td class="JQellipsis"><%=newspostVO.getTitle()%></td>
			<td class="JQellipsis"><%=newspostVO.getSummary()%></td>
			<td class="JQellipsis"><%=newspostVO.getContent()%></td>
			<td class="JQellipsis" style="width:4%;"><%=newspostVO.getNewsStatus()==1?"上架":"下架"%></td> 
			<td style="width: 10%;text-align: center;"><%=newspostVO.getNewsPostDate()%></td>
			<td><%=newspostVO.getPostComeFrom()%></td>
			<td>
			  ...
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/newspost/newspost.do" style="margin-bottom: 0px;">
			     ...
			     <input type="hidden" name="newsPostId"  value="<%=newspostVO.getNewsPostId()%>">
			     <input type="hidden" name="action"	value="getOne_For_Update">
			  </FORM>
			     
        <FORM id="deleteForm" METHOD="post" ACTION="<%=request.getContextPath()%>/newspost/newspost.do" style="margin-bottom: 0px;">
         <button class="delete" id="delete">刪除</button>
         <input type="hidden" name="newsPostId"  value="${newspostVO.newsPostId}">
