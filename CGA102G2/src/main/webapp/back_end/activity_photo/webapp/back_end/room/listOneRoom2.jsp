...
RoomVO roomVO = (RoomVO)request.getAttribute("roomVO");
  ...
  List<RoomVO> roomlist = rsvc.getAll();
  pageContext.setAttribute("roomlist", roomlist );
  
  RoomVO room = rsvc.getOneRoom(room_id);
...
<!-- 					</ul></li> -->
	...
	<!-- ==================* 主畫面區 *================== -->
	<main>
		<div id="tableall" style= "height: 440px">

<table id="table-1">
	<tr><td style= "left: 315px">
		 <h3>房間資料</h3>
		 <h4><a href="${pageContext.request.contextPath}/back_end/room/select_room_page.jsp"><img src="${pageContext.request.contextPath}/images/home.png" width="50" height="50" border="0"></a></h4>
	</td></tr>
</table>
...
<table  id="table-2" style="margin-left: 156px;">
	<tr>
		...
		<td class="value">${room.room_id}</td>
		</tr>
		<tr>
		<td class="name">➕ 房型編號</td>
		<td class="value">${room.room_type_id}</td>
		</tr>
		
		<tr>
		<td class="name">➕ 房型名稱</td>
		<td class="value">${roomtype.room_type_name}</td>
		</tr>
		
		<%-- <tr>
		<td class="name">➕ 住客名單</td>
		<td class="value">${room.room_guest_name}</td>
		</tr> --%>
		<tr>
		<td class="name">➕ 上下架狀態</td>
		<c:set var="method" value="${room.room_sale_status}" />
					<c:if test="${method == 0 }">
		...
		<tr>
		<td class="name">➕ 房間狀態</td>
		<c:set var="method" value="${room.room_status}" />
					<c:if test="${method == 1 }">
						<td class="value">已入住</td>
					</c:if>
					<c:if test="${method == 2 }">
						<td class="value">未入住</td>
					</c:if>
	</tr>
	...
	<%-- <tr>
		...
		<td>${room.room_guest_name}</td>
		<td>${room.room_sale_status}</td>
