...
List<Room_OrderVO> list = room_orderSvc.getAll();
...
<!--                 </li> -->
	...
	<div id="tableall2" style="width: 960px;">
	<table id="table-1" style="width: 960px;">
		<tr>
			<td>
				<h3>所有住宿訂單資料</h3>
				<h4>
	...
	<table  id="table-2" cellspacing="0" style="width: 950px;">
		<tr  id="tr-1">
			<th>訂單編號</th>
			<th>會員編號</th>
			<th>訂單日期</th>
		...
		<c:forEach var="room_orderVO" items="${list}" begin="<%=pageIndex%>"
			...
			<tr  style="border-bottom: none">
				...
				<c:set var="method" value="${room_orderVO.room_order_status}" />
					<c:if test="${method == 2 }">
						<td>未入住</td>
					</c:if>
					<c:if test="${method == 1 }">
				...
				<td>
					<FORM METHOD="post"
						...
						<input type="hidden"
							name="room_order_id" value="${room_orderVO.room_order_id}">
				...
				<td>
					<FORM METHOD="post"
						ACTION="<%=request.getContextPath()%>/Room_OrderServlet"
						style="margin-bottom: 0px;">
						<input class="icon" type="image" src="${pageContext.request.contextPath}/images/check-in.png"
								width="30" height="30" border="0" onclick="submit()"> 
						<input type="hidden" name="room_order_id" value="${room_orderVO.room_order_id}"> 
				...
				<td>
					<FORM METHOD="post"
						ACTION="<%=request.getContextPath()%>/Room_OrderServlet"
						style="margin-bottom: 0px;">
						<input class="icon" type="image" src="${pageContext.request.contextPath}/images/check-out.png"
								width="30" height="30" border="0" onclick="submit()"> 
						<input
							type="hidden" name="room_order_id"
							value="${room_orderVO.room_order_id}"> 
						<input
							type="hidden" name="action" value="CHECK OUT">
					</FORM>
				</td>
			</tr>
		</c:forEach>
