...
<jsp:useBean id="listActivityOrdersByCompositeQuery" scope="request" type="java.util.List<ActivityJoinVO>" /> <!-- 於EL此行可省略 -->
...
<!--                     </li> -->
        ...
        <main class="main">
            <div class="container">
                <div class="row">
                    <div class="col">
                        <h4 class="h4">活動訂單管理-活動訂單複合查詢</h4>
                    </div>
                </div>
...
<!--                                     <th scope="col">場次結束時間</th> -->
                            ...
                            <tbody class="">
								...
								<c:forEach var="activityJoinVO" items="${listActivityOrdersByCompositeQuery}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
...
<!--                                     本來想做複合查詢的顯示，但不知道為什麼資料一直跑不出來 -->
<!-- 									<td> -->
<%-- 										<c:forEach var="memberVO" items="${memberService.all}"> --%>
<%-- 						                    <c:if test="${activityJoinVO.member_id == memberVO.member_id}"> --%>
...
<%-- 						                    <c:if test="${activityJoinVO.activityID == activityVO.activity_ID}"> --%>
...
<%-- 						                    <c:if test="${activityJoinVO.activitySessionID == activitySessionVO.activity_session_ID}"> --%>
<%-- 							                    ${activitySessionVO.activity_session_ID} --%>
<%-- 						                    </c:if> --%>
<%-- 						                </c:forEach> --%>
<!-- 									</td> -->
									...
									<td>
										<c:if test="${activityJoinVO.orderState == 0}">
											<div class="border border-success rounded"><font color=green>已繳費</font></div>
										</c:if>
										<c:if test="${activityJoinVO.orderState == 1}">
											<div class="border border-primary rounded"><font color=blue>完成訂單</font></div>
										</c:if>
										<c:if test="${activityJoinVO.orderState == 2}">
											<div class="border border-danger rounded"><font color=red>取消訂單</font></div>
										</c:if>
									</td> 
									<td>
										<c:if test="${activityJoinVO.refundState == 0}">
											<div class="border border-success rounded"><font color=green>無須退款</font></div>
										</c:if>
										<c:if test="${activityJoinVO.refundState == 1}">
											<div class="border border-danger rounded"><font color=red>尚未退款</font></div>
										</c:if>
										<c:if test="${activityJoinVO.refundState == 2}">
											<div class="border border-primary rounded"><font color=blue>完成退款</font></div>
										</c:if>
									</td> 
